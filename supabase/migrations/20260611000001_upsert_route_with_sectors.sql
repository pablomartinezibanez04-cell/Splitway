-- Atomic route + sectors upsert (audit BUG-10).
--
-- The Flutter client previously replaced a route's sectors with a non-atomic
-- DELETE then INSERT issued as two separate PostgREST calls. If the INSERT
-- failed after the DELETE succeeded, the route was left with zero sectors.
-- This RPC performs the route upsert and the sector replacement inside one
-- transaction (the function body), mirroring the pattern already used for
-- sessions/free-rides.
--
-- SECURITY INVOKER (the default) so RLS and the enforce_official_owner trigger
-- still apply: a client cannot upsert a route it doesn't own, nor flip
-- is_official to true unless it is the curator account.
...
begin
  ...
  insert into public.route_templates (
    ...
    p_elevation_range_m, coalesce(p_is_official, false)
  )
